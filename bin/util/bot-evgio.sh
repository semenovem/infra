...
[ -z "$__BOT_API_EVGIO_CHAT_ID__" ] && echo "[ERRO][$0] empty __BOT_API_EVGIO_CHAT_ID__" >&2 && exit 1
[ -z "$__BOT_API_EVGIO_TOKEN__" ] && echo "[ERRO][$0] empty __BOT_API_EVGIO_TOKEN__" >&2 && exit 1
...
curl -s -X POST "https://api.telegram.org/bot${__BOT_API_EVGIO_TOKEN__}/sendMessage" \
  -d "chat_id=${__BOT_API_EVGIO_CHAT_ID__}&text=${1}" > /dev/null
